...
if __name__ == '__main__':
    ...
    x_test, y_test = read_numerical_data(
        TEST_DATA_FILE, encoding=FILE_ENCODING)
    ...
    for i in range(0, n_samples):
        # i番目のテストデータ x_test[i] をニューラルネットワーク net に入力
        output = predict(net, x_test[i])
        # ニューラルネットワークの出力に基づいて分類結果を決定（何番目の次元が最も大きい値を持つかを求める）
        z = np.argmax(output)
